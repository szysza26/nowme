...
const AvailabilitiesList = (props) => {
    ...
    const handleClickShow = (id) => {
        history.push(`/availabilities/show/${id}`);
    }
    ...
    <>
        <Paper className={classes.paper}>
            <Typography variant="h6" gutterBottom>Lista wolnych terminów: </Typography>
            ...
            <Table className={classes.table} aria-label="simple table">
                ...
                <TableBody>
                {availabilities && availabilities.map((row, index) => (
                    <TableRow key={row.id}>
                        <TableCell align="center" component="th" scope="row">{row.id}</TableCell>
                        <TableCell align="center">{row.date.substring(0, 10)}</TableCell>
                        <TableCell align="center">{row.hour_from.substring(11, 19)}</TableCell>
                        <TableCell align="center">{row.hour_to.substring(11, 19)}</TableCell>
